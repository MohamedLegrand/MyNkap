    ...
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-card w-full max-w-md rounded-2xl border border-border shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-5 border-b border-border flex items-center justify-between bg-muted/40">
          <h3 className="text-lg font-bold tracking-tight">Détail de la transaction</h3>
        ...
        {isLoading ? (
          ...
          <p className="p-6 text-sm text-destructive text-center">{error ?? 'Transaction introuvable.'}</p>
        ) : (
            ...
            {transaction.est_suspecte && (
              <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-start gap-2.5">
                ...
                <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                  Cette transaction a été signalée comme potentiellement inhabituelle. Confirmez s'il s'agit d'une erreur, ou levez le doute si elle est légitime.
                </p>
              </div>
            )}

            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Montant</span><strong className="text-foreground">{Number(transaction.montant).toLocaleString('fr-FR')} XAF</strong></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Type</span><span className="text-foreground font-semibold">{transaction.type}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Compte</span><span className="text-foreground font-semibold">{nomCompte(transaction.id_compte)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Catégorie</span><span className="text-foreground font-semibold">{nomCategorie(transaction.id_categorie)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Date</span><span className="text-foreground font-semibold">{new Date(transaction.date).toLocaleDateString('fr-FR')}</span></div>
              {transaction.description && (
                <div className="flex justify-between gap-3"><span className="text-muted-foreground shrink-0">Description</span><span className="text-foreground font-semibold text-right">{transaction.description}</span></div>
              )}
              <div className="flex justify-between"><span className="text-muted-foreground">Enregistrée le</span><span className="text-foreground font-semibold">{new Date(transaction.date_creation).toLocaleString('fr-FR')}</span></div>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              {peutAnnuler && (
                ...
                >
                  ...
                  <span>Annuler cette transaction</span>
                </button>
              )}
            </div>
          </div>
